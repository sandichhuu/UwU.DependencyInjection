...
namespace UwU.DI.Injection
{
    ...
    public class Injector : IInjector
    {
        ...
        {
            var instance = this.container.First(fieldInfo.FieldType);
            ...
            {
                this.logger?.Error($"Inject {target.GetType().Name}->{fieldInfo.Name} failed !");
            }
        }

        private void InjectProperty(object target, PropertyInfo propertyInfo)
        {
            var instance = this.container.First(propertyInfo.PropertyType);
            ...
            {
                this.logger?.Error($"Inject {target.GetType().Name}->{propertyInfo.Name} failed !");
            }
        }
        ...
        private void InjectMethod(object target, MethodInfo methodInfo)
        {
            ...
            for (var i = 0; i < length; i++)
            {
                ...
                var instance = this.container.First(param.ParameterType);
                ...
                else
                {
                    ...
                    this.logger?.Warn($"Inject {target.GetType().Name}->{methodInfo.Name}->{param.Name} failed !");
                }
            }
